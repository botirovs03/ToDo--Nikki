...
router.post("/api/categories", authenticateUser, async (req, res) => {
  ...
  try {
    ...
    const checkCategoryQuery = `
        ...
        `;
    const categoryCheckResult = await connection
      .promise()
      .query(checkCategoryQuery, [userID, categoryName]);
    ...
    const insertResult = await connection
      .promise()
      .query(insertCategoryQuery, [userID, categoryName]);
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const categoryCheckResult = await connection
        .promise()
        .query(checkCategoryQuery, [categoryID, userID]);
      ...
      await connection
        .promise()
        .query(updateCategoryQuery, [categoryName, categoryID]);
